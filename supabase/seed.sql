...
begin
  ...
  -- ON CONFLICT DO NOTHING, and the reason is worth reading: since migration
  -- 057 the class_series insert above FIRES A TRIGGER that materialises twelve
  -- months forward. The forward half of this range is therefore already there
  -- by the time this statement runs, and generated by the real job rather than
  -- by the seed — which is the right way round. What is left here is the
  -- HISTORY, which the generator will not create and the AI features need.
  --
  -- The target is named rather than left bare: a bare ON CONFLICT DO NOTHING
  -- tries every constraint as an arbiter, including the DEFERRABLE instructor
  -- exclusion, and Postgres refuses ("does not support deferrable ... as
  -- arbiters"). Both unique indexes catch the same rows here because the seed
  -- sets series_slot_at = starts_at, so either will do.
  insert into class_occurrences
    (studio_id, location_id, series_id, class_type_id, name, instructor_id,
     room_id, capacity, starts_at, ends_at, status, series_slot_at)
