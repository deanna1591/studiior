-- =============================================================================
-- MIGRATION 002 — book_class()
--
-- The booking transaction. Business Rules §2 (eligibility, payment source,
-- overrides), §4 (waitlist), §5 (capacity), §6 (credits).
--
-- The whole point of this function is that it is ONE transaction holding a row
-- lock on the occurrence. Data Model §6 concurrency rule: the lock is taken
-- before anything reads booked_count. There is no application-level path to
-- creating a booking; check-then-insert overbooks under load.
--
-- security definer, because a member booking themselves must write a
-- credit_ledger row and bump class_occurrences.booked_count, and neither is
-- reachable under their own RLS policies. The function therefore does its own
-- authorisation in §0 below. It runs as the table owner, so RLS is bypassed
-- inside — every read is explicitly scoped to the occurrence's studio_id.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- Return composite. failure_reason is null on success; on failure every other
-- field is null. Reason codes are specific by design — Business Rules §2.1
-- forbids a generic "cannot book".
-- -----------------------------------------------------------------------------
create type book_class_result as (
  booking_id        uuid,
...
comment on type book_class_result is
  ...
  'member_wrong_studio, not_authorised, class_cancelled, class_completed, '
  'class_in_past, outside_booking_window, past_booking_cutoff, '
  'waiver_not_signed, member_not_active, already_booked, daily_limit_reached, '
  'future_limit_reached, class_type_not_in_plan, class_full, waitlist_closed.';
...
begin
  ...
  loop
    ...
    v_pay        := case when v_cand.priority = 1
                         then 'membership'::payment_source
                         when v_cand.priority = 2
                         then 'membership'::payment_source
  ...
  if v_full and not v_override then
    ...
    -- (§4.2.4), so it is deliberately left null on the row.
...
comment on function book_class(uuid, uuid, booking_source, text) is
  ...
  'credit_ledger row and booked_count together. Never call the underlying '
  'tables directly to create a booking.';

-- Members and staff both call this; row-level authorisation is inside the
-- function, not in the grant. `anon` is deliberately not granted.
revoke all on function book_class(uuid, uuid, booking_source, text) from public;
grant execute on function book_class(uuid, uuid, booking_source, text)
