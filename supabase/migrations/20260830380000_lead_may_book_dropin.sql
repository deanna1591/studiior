-- =============================================================================
-- MIGRATION 028 — Decision 15: a `lead` may book, drop-in only
--
-- §2.1 rule 5 is "member status is active", which turns away the walk-in who
-- found the studio on their phone on Tuesday evening and wants tomorrow's 7am.
-- That is the best new member a studio gets, refused at the moment their
-- intent is highest, because nobody has flipped a flag for them yet.
--
-- Rule 5 now passes `lead` as well as `active`. `inactive` and `archived` are
-- unchanged and still refused.
--
-- Rule 5 runs BEFORE §2.2 works out who pays, so allowing a lead through there
-- alone would also let one book against a membership. A second guard runs
-- after resolution: a lead resolving to anything other than drop_in is refused
-- with the same reason code. In practice a lead has bought nothing and always
-- resolves to drop-in; the guard is for the case where staff attach a plan to
-- somebody without activating them, and it keeps `lead` from quietly becoming
-- a way to spend credits nobody sold.
--
-- Attending does not promote a lead. Status is what staff set when they sell
-- something, and a booking that is never paid for at the desk should not leave
-- a lead looking like a paying member.
--
-- This is a behaviour change to the booking gate, so it is Decision 15 in the
-- log rather than an edit nobody agreed to.
-- =============================================================================

create or replace function book_class_status_ok(p_status member_status)
returns boolean language sql immutable as $$
  select p_status in ('active', 'lead')
$$;

comment on function book_class_status_ok(member_status) is
  'Business Rules §2.1 rule 5, as amended by Decision 15. A lead passes here '
  'and is then held to drop-in by the second guard after §2.2 resolution.';

revoke execute on function book_class_status_ok(member_status) from public;
grant execute on function book_class_status_ok(member_status) to authenticated, service_role;

-- The gate lives inside book_class(). Replaced forward from the LIVE
-- definition rather than from migration 002's text: migration 003 added
-- p_payment_source, so re-issuing 002's four-argument signature creates a
-- SECOND overload and every call then fails as ambiguous. Which is exactly
