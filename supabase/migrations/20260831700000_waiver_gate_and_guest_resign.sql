-- =============================================================================
-- 165  Decision 34 Part B: the guest door, the checklist item, and the
--      no-version state.
-- =============================================================================
--  1. enforce_guest_waiver gains the re-sign check at check-in — a guest who
--     signed only an older required version is turned away like an unsigned one.
--  2. studio_setup_state gains a 'waiver' item, present only when require_waiver
--     is on: outstanding until a version is published.
--  3. The no-version state: book_class returns 'waiver_unavailable' (a distinct
--     reason pointing at the studio) when require_waiver is on and no version
--     exists; the app says the studio hasn't published its waiver yet rather than
--     offering a screen with nothing on it.
--
-- book_class is re-issued from migration 164 — the newest definition, found with
-- scripts/newest-definition.sh book_class (20260831690000).
-- =============================================================================

-- -----------------------------------------------------------------------------
-- 1. enforce_guest_waiver — unsigned OR stale-required-version is refused at
--    check-in. Re-issued from migration 132 (guest_passes). Scoped to guests
--    (the guest_passes join), so an ordinary member's check-in is untouched —
--    their re-sign is enforced at BOOKING (book_class 2.1.4).
-- -----------------------------------------------------------------------------
create or replace function enforce_guest_waiver() returns trigger
language plpgsql security definer set search_path = public as $$
begin
  -- Unsigned entirely — the original gate.
  if exists (
  ...
  -- Decision 34 Part B: signed, but on an OLDER version while the current one
  -- requires re-signing. Same treatment as unsigned.
  if exists (
    select 1 from guest_passes gp
     where gp.guest_member_id = new.member_id
       and gp.occurrence_id  = new.occurrence_id
...
-- -----------------------------------------------------------------------------
-- 2. studio_setup_state gains a 'waiver' item — present ONLY when require_waiver
--    is on (like 'publish' for publication), outstanding until a version exists.
--    Re-issued from migration 155.
-- -----------------------------------------------------------------------------
...
returns jsonb language sql stable security definer set search_path = public as $$
  ...
  facts(key, done) as (
    values
      ...
      -- Decision 34: publish a waiver. Done once a version exists; only present
      -- for a studio that requires a waiver (the WHERE clause below).
      ('waiver',
       exists (select 1 from waiver_versions where studio_id = p_studio_id)),
      ('connect_stripe',
  ...
  select jsonb_object_agg(
   ...
   where (exists (select 1 from studio_staff s
     ...
     -- The waiver item exists only for a studio that requires a waiver.
     and (f.key <> 'waiver'
          or coalesce((select require_waiver from studio_settings where studio_id = p_studio_id), false));
$$;

-- -----------------------------------------------------------------------------
-- 3. book_class rule 2.1.4 — 'waiver_unavailable' when require_waiver is on and
--    no version exists. Re-issued from migration 164 (the newest book_class).
-- -----------------------------------------------------------------------------

-- -----------------------------------------------------------------------------
-- 0. current_waiver gains require_waiver, so the member Home can tell a studio
--    that requires a waiver but has published none from one that does not care.
--    Re-issued from migration 164.
-- -----------------------------------------------------------------------------
create or replace function current_waiver(p_studio_id uuid) returns jsonb
language plpgsql stable security definer set search_path = public as $$
declare v waiver_versions%rowtype; v_member uuid; v_signed boolean := false; v_req boolean;
begin
  ...
  select coalesce(require_waiver, false) into v_req from studio_settings where studio_id = p_studio_id;
  select * into v from waiver_versions where studio_id = p_studio_id
   order by created_at desc limit 1;
  if not found then
    return jsonb_build_object('exists', false, 'require_waiver', coalesce(v_req, false));
  end if;
  ...
  return jsonb_build_object(
    'exists', true, 'require_waiver', coalesce(v_req, false),
    'version_id', v.id, 'format', v.format,
...
-- create-or-replace keeps book_class's ACL; re-assert.
