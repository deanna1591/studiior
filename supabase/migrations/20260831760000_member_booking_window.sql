-- Migration 171 — Decision 36: the member app's booking horizon is the MEMBER's.
-- book_class rule 2.1.2 resolves the window from the member's highest-priority
-- usable plan, coalesced to the studio default. member_bootstrap only ever
-- exposed coalesce(studio_settings.booking_window_days, 30) — studio only — so
-- the "Opens for booking" screen said a class opens LATER than book_class would
-- have accepted it. Extract 2.1.2 into ONE function the rule and the screen both
-- read, so they cannot diverge. Also: the column had no settings UI (only the
-- onboarding wizard wrote it) — a screen is added in Settings -> Timetable.

-- --- 1. The single definition of the §2.1.2 window ---------------------------
create function member_booking_window_days(p_member_id uuid) returns int
...
declare
  ...
  v_window int;
begin
  select studio_id, user_id into v_studio, v_owner from members where id = p_member_id;
  if v_studio is null then
    raise exception 'no such member' using errcode = 'PT404';
  end if;
  -- The caller must be the member, staff of that studio (is_desk_up), or a
  -- trusted backend role. This mirrors book_class's OWN permission EXACTLY
  -- (v_trusted = is_service_context(), OR is_desk_up, OR is_self), so book_class
  -- calling this can never refuse a booking it would otherwise allow.
  if not (is_service_context()
          or coalesce(v_owner = auth.uid(), false)
          or coalesce(is_desk_up(v_studio), false)) then
    raise exception 'that is not your booking window' using errcode = 'PT403';
  end if;

  select timezone into v_tz from studios where id = v_studio;
  v_today := (now() at time zone v_tz)::date;

  -- book_class rule 2.1.2's selection, verbatim: the highest-priority usable
  -- plan (recurring > trial > pack, earliest expiry) carrying a window OR a
  -- daily cap. The OR-cap condition matters — book_class picks THIS same row for
  -- its cap, so a plan chosen for its cap with a null window must fall through to
  -- the studio here too. Coalesce to the studio value (never a hardcoded 30):
  -- null means the studio set none and no plan carries one, which book_class
  -- treats as "no limit", and the screen must render honestly rather than invent.
  select mp.booking_window_days into v_window
    from memberships ms
    join membership_plans mp on mp.id = ms.plan_id
   where ms.member_id  = p_member_id
     and ms.studio_id  = v_studio
  ...
  return coalesce(v_window,
                  (select booking_window_days from studio_settings where studio_id = v_studio));
end $$;
revoke execute on function member_booking_window_days(uuid) from public, anon;
grant  execute on function member_booking_window_days(uuid) to authenticated, service_role;

-- --- 2. book_class re-issued to call it (window only; cap keeps its own read) -
...
-- create-or-replace keeps book_class's ACL; re-assert to be explicit.
...
-- --- 3. member_bootstrap resolves booking_window_days through the same fn -----
