...
**Built in migration 057, and it needed one column this section did not anticipate.** `is_exception` is not sufficient on its own: an occurrence moved to another day leaves no row in its original slot, so a generator keyed on `(series_id, starts_at)` refills it and the member gets two classes. `class_occurrences.series_slot_at` records which recurrence a row materialises and survives the move, so the row keeps holding its origin; the unique index on `(series_id, series_slot_at)` is what makes regeneration idempotent. The horizon is `studio_settings.occurrence_horizon_days`, **default 60** — this month and the next, which is how a boutique studio plans and the month migration 066's availability cycle collects for. It was `occurrence_horizon_months` default 12 until migration 068, and **no screen had ever read or written it**, which is why one studio was carrying 1,421 open classes fourteen months out that any member could book. It is now at `/settings`, and changing it TRIMS as well as extends: shortening it used to do nothing at all, because `generate_occurrences()` only ever inserts. What falls outside is **deleted, not cancelled** — a cancelled row keeps its `series_slot_at`, and the unique index on it would make the hole permanent if the studio ever lengthened the horizon again. Nothing with a booking or a check-in is touched: the whole change is refused and names them. Nor is anything somebody has moved (`is_exception`), assigned by hand (`assigned_by`), or created as a one-off with no series. *This and future* editing is built in migration 064 (`update_series()`); *entire series* is deliberately not offered, because it would rewrite the past.
