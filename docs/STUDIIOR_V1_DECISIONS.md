...
**5. Roster and records.** The staff and instructor rosters show the **method per checked-in member** (self / scanned / desk) and, **for self, the distance** — a derived metres figure, never the raw lat/lng, which is treated as member data (see §14 flag). The desk can correct a self check-in to `no_show` as today. **Reports are unchanged** — a check-in is a check-in whoever made it, and pay never read attendance anyway.
...
- **The member waiver-at-the-door is a behaviour change for the DESK and INSTRUCTOR too, and that has to be an intentional choice.** Decision 34 put the member re-sign check at **booking** (`book_class` 2.1.4) and the *guest* re-sign check at the **door** (`enforce_guest_waiver`, scoped to the `guest_passes` join). Extending `enforce_guest_waiver` "so members are refused like guests" means the trigger fires on **every** `check_ins` insert for a member — so a member whose studio just turned on `require_waiver` + `requires_resign` with existing bookings would be refused at the **desk and by an instructor scan**, not only self check-in. That is defensible (a stale waiver is stale whoever scans them) and probably what the studio wants, but it is a change from today, where a member is gated only at booking. **Two readings, pick one in Part A:** (a) keep the door member-gate inside `self_check_in` only (self door), leaving desk/instructor unchanged; or (b) extend the trigger to all doors for members. The brief's wording ("extend enforce_guest_waiver … like guests") is reading (b); the entry adopts **(b)** as the more correct one, and it is recorded so it is not discovered as a surprise at the desk. `self_check_in` also checks the waiver itself (belt-and-braces) so the reason is a clean sentence rather than a raw trigger error.

- **§14 and the coordinates.** §14 keeps a member's contact details and address with the office and denies them to instructors. A check-in **distance** ("40 m") is an operational fact about this check-in, not a contact detail, so surfacing it to the instructor on the roster is fine. The **raw lat/lng** is closer to member location data: it is stored on `check_ins` (nullable) but **only the derived distance is surfaced**, and the raw coordinates are kept out of `instructor_roster()` (which already withholds §14 fields) — desk/manager may read them, an instructor may not. Recorded so the roster reader does not start returning raw coordinates.
...
**Where:** Permissions §8 (amended) and §14; Business Rules §8; Data Model §5 (`locations`, `check_ins`); Decisions 25 (published month), 28 (distinct), 32 (pay unaffected), 34 (waiver at the door). **Status:** entry written, build not started. **Build in two halves, each through the full loop.** (A) the enum migration (`'instructor'`, `'import'`); `locations` coordinates + radius + switch + the Settings UI; `self_check_in` with geofence + window + member waiver-at-door; the Home card button + reasons as sentences; coordinates on `check_ins`; roster method display; tests (own booking only; window closed → refused; too far → `too_far` with accuracy honoured; no location → `no_location`; studio without location → button absent; waiver stale → PT422; idempotent; another member's booking → PT403; `all_off` unchanged); browser-verify at phone width with mocked geolocation inside and outside the radius. (B) the printed studio QR (slug, print view, re-mint) + the `/checkin/{slug}` route; `instructor_resolve_code` + camera scan on the roster + `method = 'instructor'`; tests; full loop.
