...
/**
 * The PLATFORM webhook — Studiior's own account, not a studio's.
 *
 * A SECOND endpoint with a SECOND signing secret, because platform events and
 * Connect events are different deliveries and must not be able to be mistaken
 * for one another. stripe_platform_webhook() refuses anything carrying an
 * `account` field; the Connect one refuses anything without.
 *
 ...
 * the signature header to stripe_platform_webhook(), which recomputes the HMAC in the
...
export async function POST(request: NextRequest) {
  ...
  const { data, error } = await supabase.rpc("stripe_platform_webhook", {
    p_payload: payload,
